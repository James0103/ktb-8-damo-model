...
class UserData(BaseModel):
    ...
    is_push_notification_allowed: bool = Field(default=False, description="푸시 알림 허용 여부(계획적인지 아닌지)")
    created_at: datetime = Field(..., description="생성 일시")
    updated_at: datetime = Field(..., description="수정 일시")
    withdraw_at: Optional[datetime] = Field(None, description="사용자 탈퇴 일시(탈퇴와 리뷰를 연관지어서 식당이 마음에 안들었을수도 있음)")
